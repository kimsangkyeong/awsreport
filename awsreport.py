...
def df_to_excel(writer, df, sheetname):
  ...
  header_format = workbook.add_format({'bold': True, 'valign': 'vcenter', 'text_wrap': True, 'fg_color': '#FDE9D9', 'border': 1})
...
def main(argv):
  ...
  #   with pd.ExcelWriter(output_file, mode='a', if_sheet_exists='replace', engine='xlsxwriter') as writer:
  #     df_to_excel(writer, df_route53           , 'route53')                     # 1
  #     df_to_excel(writer, df_route53_record    , 'route53_record')              # 2
